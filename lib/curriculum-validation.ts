...
const PLACEHOLDER_TITLES = new Set(["new lesson", "untitled video lesson", "untitled reading", "untitled quiz"])

export function getCurriculumIssues(
  course: Pick<AdminCourseDetail, "sections" | "thumbnailUrl" | "description" | "price">
): string[] {
  const issues: string[] = []
  const lessons = course.sections.flatMap((s) => s.lessons)

  if (!course.thumbnailUrl) issues.push("Course thumbnail is missing")
  ...
  if (Number(course.price) === 0 && freePreviews.length === lessons.length && lessons.length > 0) {
...
export function getCurriculumPublishIssues(course: AdminCourseDetail): string[] {
  return getCurriculumIssues(course)
}
